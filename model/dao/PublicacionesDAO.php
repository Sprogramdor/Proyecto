...
class PublicacionesDAO {
  ...
  public function selectOne($id) { // buscar un producto por su id
    ...
    public function update($prod){
        ...
        try{
            //prepare
            $sql = "UPDATE Publicacion SET num_publicacion=:num, `publi_titulo=:titulo`, `publi_categoria=:categoria`, `publi_detalle=:detalle`, `publi_usuarioActualizacion=:usuario`, `publi_fechaActualizacion=:fechaA`, `fechapublicacion=:fecha`";
           //bind parameters
            $sentencia = $this->con->prepare($sql);
            $data = [
                'num' =>  $publicaciones->getnum(),
   ...
   public function delete($publicaciones){
       try{
        //prepare
        $sql = " DELETE FROM PUBLICACION WHERE prod_id=:id";
        $sentencia = $this->con->prepare($sql);
        $data = [
